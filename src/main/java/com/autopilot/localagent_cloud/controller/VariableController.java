...
import com.autopilot.localagent_cloud.auth.JwtUtil;
...
public class VariableController {
    ...
    public ResponseEntity<List<Variable>> getAll(
        ...
        // Mask secret values
        vars.forEach(v -> { if (Boolean.TRUE.equals(v.getIsSecret())) v.setValue("â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢"); });
    ...
    public ResponseEntity<Variable> update(@PathVariable Long id, @RequestBody Variable body, HttpServletRequest request) {
        return variableRepository.findById(id).map(v -> {
            ...
            if (body.getValue() != null && !body.getValue().equals("â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢")) {
