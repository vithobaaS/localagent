...
public class KeyUpAction implements ActionHandler {
    @Override
    public void execute(WebDriver driver, TestStep step, ExecutionService context) throws Exception {
        new org.openqa.selenium.interactions.Actions(driver).keyUp(org.openqa.selenium.Keys.valueOf(step.data.toUpperCase())).perform();
