              ...
              {steps.map((s, i) => (
                <div key={i} className="step-card">
                  ...
                  <div className="step-body">
                    <div className="step-field"><span className="step-field-label">Action *</span><input list={`action-list-${i}`} className="form-input" placeholder="Type action..." value={s.actionName} onChange={e => updateStep(i, 'actionName', e.target.value)} required /><datalist id={`action-list-${i}`}>{ACTIONS.map(a => <option key={a} value={a}>{a}</option>)}</datalist></div>
